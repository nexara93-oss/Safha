  ...
  return (
    ...
    >
      <svg
        ...
        className={`relative block h-[60px] w-full sm:h-[90px] md:h-[120px] ${flip ? "rotate-180" : ""}`}
      >
        <path
...
/**
 * Multi-layer wave for richer transitions
 */
export function WaveDividerMulti({
  fromColor = "#2A4DFF",
  toColor = "#FAFAFA",
  flip = false
}: {
  ...
  return (
    <div className="relative w-full overflow-hidden leading-none" style={{ backgroundColor: fromColor }} aria-hidden>
      <svg
        viewBox="0 0 1200 120"
        preserveAspectRatio="none"
        className={`relative block h-[80px] w-full sm:h-[110px] md:h-[140px] ${flip ? "rotate-180" : ""}`}
      >
        <path
