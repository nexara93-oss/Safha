    ...
    <DashboardShell allowedRoles={["STUDENT"]}>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-brand-ink/60 dark:text-brand-paper/60">{t("student.timetable.subtitle")}</p>
        </div>

        {slots.length === 0 ? (
          <div className="card flex flex-col items-center gap-2 py-12 text-center">
            <Clock className="h-10 w-10 text-gray-300" />
            <p className="text-sm text-gray-500">{t("student.timetable.noSlots")}</p>
          </div>
        ) : (
